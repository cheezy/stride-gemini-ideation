...
# and wired into commands/ideate.toml (the Step-6 "## Design challenge" template).
# The gate is an interactive question step — surfaced through Gemini CLI's
# question UI (not Claude Code's AskUserQuestion) — that cannot be driven from a
# non-interactive runner, so this test asserts the *output contract* (the shape
# a committed requirements doc must exhibit once the gate has run) against the
# committed fixture
...
# template in commands/ideate.toml. If you change the documented gate output,
